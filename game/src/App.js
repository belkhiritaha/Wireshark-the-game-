...
function App() {
  ...
  async function generateFish(n, x, y, z) {
    const fishList = [];
    for (var i = 0; i < n; i++) {
      const fish = await loadGLTF('http://localhost:8000/res/models/piranhahigh.gltf');
  ...
  async function generateFishes(fishListList) {
    for (var i = 0; i < 10; i++) {
  ...
  async function game(fishListList) {
    ...
    const shark = await loadGLTF('http://localhost:8000/res/models/shark.glb');
    console.log(shark);
  ...
  async function load() {
    await generateFishes(fishListList);
  }

  useEffect(() => {
    if (loading || fishListList.length == 0) {
      load().then(() => {
        setLoading(false);
      }
      );
  ...
  if (loading){
    return(
      <div style={{position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", fontSize: "50px"}}>
        <h1>Loading...</h1>
        <p>the game has some missing features</p>
      </div>
    )
  }
  else{
    game(fishListList);
    if(fishListList.length > 0){
    }
  }
}
